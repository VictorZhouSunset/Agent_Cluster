...
Maps normalized providers onto `/api/*` endpoints with structured errors and request failure logging.
...
| appRouter.ts | central router | dispatches API requests, maps shared errors, and logs unexpected failures |
...
| internalChannels.ts | endpoint handlers | serves internal-only OpenClaw config read/write/reload routes plus Telegram compatibility handlers |

...
- serves a dashboard-facing HTTP API for `agent_1`
- reads and writes allowlisted files directly on `agent_2`
...
Deploy this entire folder to `agent_2`:
...
- `DASHBOARD_ADAPTER_SECRET`
  The secret expected from the dashboard caller on `agent_1` in header `X-Dashboard-Secret`.
...
- `DASHBOARD_ADAPTER_ROOT_DIR`
  Root directory on `agent_2` whose allowlisted files and `skills/*/SKILL.md` entries should be exposed.
...
- `DASHBOARD_ADAPTER_CONTROL_URL`
  The local `9001` control-plane base URL on `agent_2`.
...
- allow inbound `9011` from `agent_1` only in the EC2 Security Group
- send `X-Dashboard-Secret` from the dashboard backend on `agent_1`
